...
print_results(SQLHSTMT hStmt) 
{
	...
	do {
		...
		for (c=0; c < ncols; c++) {
			...
			name[namelen] = '\0';
			metadata[c].name = tds_strdup((char *) name);
...
get_login(int argc, char *argv[], OPTIONS *options)
{
	...
	while ((ch = getopt(argc, argv, "U:P:S:dD:i:o:e:t:hqv")) != -1) {
		switch (ch) {
		case 'U':
			login->username = tds_strdup(optarg);
			break;
		case 'P':
			login->password = tds_strdup(optarg);
			break;
		case 'S':
			options->servername = tds_strdup(optarg);
			break;
		case 'd':
		case 'D':
			options->database = tds_strdup(optarg);
			break;
		case 'i':
			options->input_filename = tds_strdup(optarg);
			break;
		case 'o':
			options->output_filename = tds_strdup(optarg);
			break;
		case 'e':
			options->error_filename = tds_strdup(optarg);
			break;
		case 't':
			unescape(optarg);
			options->colsep = tds_strdup(optarg);
			break;
		case 'h':
