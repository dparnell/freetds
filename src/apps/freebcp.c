...
process_parameters(int argc, char **argv, BCPPARAMDATA *pdata)
{
	...
	pdata->dbobject = tds_strdup(argv[1]);
	...
	while ((ch = getopt(argc, argv, "m:f:e:F:L:b:t:r:U:P:I:S:h:T:A:O:0:ncEdvV")) != -1) {
		...
		case 'e':
			pdata->eflag++;
			pdata->errorfile = tds_strdup(optarg);
			break;
		case 'F':
			pdata->Fflag++;
		...
		case 't':
			pdata->tflag++;
			pdata->fieldterm = tds_strdup(optarg);
			pdata->fieldtermlen = unescape(pdata->fieldterm);
			break;
		case 'r':
			pdata->rflag++;
			pdata->rowterm = tds_strdup(optarg);
			pdata->rowtermlen = unescape(pdata->rowterm);
			break;
		case 'U':
			pdata->Uflag++;
			pdata->user = tds_strdup(optarg);
			break;
		case 'P':
			pdata->Pflag++;
			if ((strcmp(optarg, "-")) == 0) {
				...
				nl = strchr(pwd, '\n');
				if(nl) *nl = '\0';
				pdata->pass = tds_strdup(pwd);
			} else {
				pdata->pass = tds_strdup(optarg);
			}
			break;
		case 'I':
			pdata->Iflag++;
		...
		case 'S':
			pdata->Sflag++;
			pdata->server = tds_strdup(optarg);
			break;
		case 'h':
			pdata->hint = tds_strdup(optarg);
			break;
		case 'O':
		case '0':
			pdata->options = tds_strdup(optarg);
			break;
		case 'T':
			pdata->Tflag++;
