...
populate_login(TDSLOGIN * login, int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "H:S:I:P:U:p:Co:t:r:D:")) != -1) {
		switch (opt) {
		case 't':
			opt_col_term = tds_strdup(optarg);
			break;
		case 'r':
			opt_row_term = tds_strdup(optarg);
			break;
		case 'D':
			opt_default_db = tds_strdup(optarg);
			break;
		case 'o':
...
main(int argc, char **argv)
{
	...
	if (context->locale && !context->locale->date_fmt) {
		...
		context->locale->date_fmt = tds_strdup(STD_DATETIME_FMT);
	}
	...
	for (s=NULL, s2=NULL; ; free(s), free(s2), s2=NULL) {
		...
		if ((s2 = tds_strdup(s)) == NULL) {	/* copy to mangle with strtok() */
			perror("tsql: ");
			exit(1);
		}
