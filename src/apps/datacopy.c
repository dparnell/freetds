		...
		case GET_NEXTARG:
			...
			case 'S':
				...
				if (strlen(arg) > 2) {
					...
					if (!tok)
						return FALSE;
					pdata->sserver = tds_strdup(tok);

					tok = strtok(NULL, "/");
					if (!tok)
						return FALSE;
					pdata->suser = tds_strdup(tok);

					tok = strtok(NULL, "/");
					if (!tok)
						return FALSE;
					pdata->spass = tds_strdup(tok);

					tok = strtok(NULL, "/");
					if (!tok)
						return FALSE;
					pdata->sdb = tds_strdup(tok);

					tok = strtok(NULL, "/");
					if (!tok)
						return FALSE;
					pdata->sdbobject = tds_strdup(tok);
			...
			case 'D':
				...
				if (strlen(arg) > 2) {
					...
					if (!tok)
						return FALSE;
					pdata->dserver = tds_strdup(tok);

					tok = strtok(NULL, "/");
					if (!tok)
						return FALSE;
					pdata->duser = tds_strdup(tok);

					tok = strtok(NULL, "/");
					if (!tok)
						return FALSE;
					pdata->dpass = tds_strdup(tok);

					tok = strtok(NULL, "/");
					if (!tok)
						return FALSE;
					pdata->ddb = tds_strdup(tok);

					tok = strtok(NULL, "/");
					if (!tok)
						return FALSE;
					pdata->ddbobject = tds_strdup(tok);
