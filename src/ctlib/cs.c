...
_cs_locale_copy_inplace(CS_LOCALE *new_locale, CS_LOCALE *orig)
{
	...
	if (orig->language) {
		new_locale->language = tds_strdup(orig->language);
		if (!new_locale->language)
			goto Cleanup;
	}

	if (orig->charset) {
		new_locale->charset = tds_strdup(orig->charset);
		if (!new_locale->charset)
			goto Cleanup;
	}

	if (orig->time) {
		new_locale->time = tds_strdup(orig->time);
		if (!new_locale->time)
			goto Cleanup;
	}

	if (orig->collate) {
		new_locale->collate = tds_strdup(orig->collate);
...
TDSCONTEXT *tds_ctx;
	...
	*ctx = (CS_CONTEXT *) malloc(sizeof(CS_CONTEXT));
	...
	if (tds_ctx->locale && !tds_ctx->locale->date_fmt) {
		...
		tds_ctx->locale->date_fmt = tds_strdup(STD_DATETIME_FMT);
	}
	return CS_SUCCEED;
}
