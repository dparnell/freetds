...
bcp_init(DBPROCESS * dbproc, const char *tblname, const char *hfile, const char *errfile, int direction)
{
	...
	if (hfile != NULL) {
		...
		if (dbproc->hostfileinfo == NULL)
			goto memory_error;
		if ((dbproc->hostfileinfo->hostfile = tds_strdup(hfile)) == NULL)
			goto memory_error;

		if (errfile != NULL)
			if ((dbproc->hostfileinfo->errorfile = tds_strdup(errfile)) == NULL)
				goto memory_error;
	} else {
	...
	if ((dbproc->bcpinfo->tablename = tds_strdup(tblname)) == NULL)
		goto memory_error;
	...
	if (direction == DB_IN) {
		...
		for (i = 0; i < bindinfo->num_cols; i++) {
			...
			if (resinfo->columns[i]->table_column_name)
				curcol->table_column_name = tds_strdup(resinfo->columns[i]->table_column_name);
...
bcp_columns(DBPROCESS * dbproc, int host_colcount)
{
	...
	dbproc->hostfileinfo->host_columns = calloc(host_colcount, sizeof(BCP_HOSTCOLINFO *));
	...
	for (i = 0; i < host_colcount; i++) {
		dbproc->hostfileinfo->host_columns[i] = calloc(1, sizeof(BCP_HOSTCOLINFO));
 ...
 * \sa 	bcp_control(), 
 * 	bcp_exec(), 
 * \todo Simplify.  Remove \a valuelen, and dbproc->bcpinfo->hint = tds_strdup(hints[i])
 */
RETCODE
...
_bcp_measure_terminated_field(FILE * hostfile, BYTE * terminator, int term_len)
{
	...
	if ((sample = malloc(term_len)) == NULL) {
...
bcp_readfmt(DBPROCESS * dbproc, char *filename)
{
	...
	while ((_bcp_fgets(buffer, sizeof(buffer), ffile)) != NULL) {
		...
		if (topptr == NULL) {	/* first time */
			if ((curptr = malloc(sizeof(struct fflist))) == NULL) {
				dbperror(dbproc, SYBEMEM, errno);
				return (FAIL);
			}
			topptr = curptr;
		} else {
			if ((curptr->nextptr = malloc(sizeof(struct fflist))) == NULL) {
				dbperror(dbproc, SYBEMEM, errno);
				return (FAIL);
			}
