...
odbc_get_msg(const char *sqlstate)
{
	...
	/* TODO set flag and use pointers (no tds_strdup) ?? */
	while (pmap->msg) {
		if (!strcasecmp(sqlstate, pmap->sqlstate)) {
			return tds_strdup(pmap->msg);
		}
		++pmap;
	}
	return tds_strdup("");
...
odbc_errs_add(struct _sql_errors *errs, const char *sqlstate, const char *msg)
{
	...
	errs->errs[n].server = tds_strdup("DRIVER");
	errs->errs[n].msg = msg ? tds_strdup(msg) : odbc_get_msg(errs->errs[n].state3);
	++errs->num_errors;
}
		    ...
		    const char *server)
{
	...
	/* TODO why driver ?? -- freddy77 */
	errs->errs[n].server = (server) ? tds_strdup(server) : tds_strdup("DRIVER");
	errs->errs[n].msg = msg ? tds_strdup(msg) : odbc_get_msg(errs->errs[n].state3);
	       ...
	       SQLINTEGER FAR * pfNativeError, SQLCHAR * szErrorMsg, SQLSMALLINT cbErrorMsgMax, SQLSMALLINT FAR * pcbErrorMsg)
{
	...
	p = malloc(strlen(msgprefix)+strlen(msg)+1);
	if (sprintf(p, "%s%s", msgprefix, msg) < 0)
		return SQL_ERROR;
