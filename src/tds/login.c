...
tds_save(TDSSAVECONTEXT *ctx, char type, TDSMESSAGE *msg)
{
	...
	dest_msg->type = type;
	dest_msg->msg = *msg;
#define COPY(name) if (msg->name) dest_msg->msg.name = tds_strdup(msg->name);
	COPY(server);
	COPY(message);
	COPY(proc_name);
	COPY(sql_state);
...
tds_save_env(TDSSOCKET * tds, int type, char *oldval, char *newval)
{
	...
	env->type = type;
	env->oldval = oldval ? tds_strdup(oldval) : NULL;
	env->newval = newval ? tds_strdup(newval) : NULL;
	++ctx->num_env;
}
...
tds_connect(TDSSOCKET * tds, TDSCONNECTION * connection)
{
	 ...
	 * tds->date_fmt=tds_strdup(connection->date_fmt);
	 * }
	 */
